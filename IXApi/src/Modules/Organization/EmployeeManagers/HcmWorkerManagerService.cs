...
{
    public class HcmWorkerManagerService : BaseService<HcmWorkerManager>, IHcmWorkerManagerService
    {
        public HcmWorkerManagerService(IUnitOfWork unitOfWork, ICurrentUserService currentUser) : base(unitOfWork, currentUser)
        {
        }

        public async Task<IEnumerable<HcmWorkerManagerDto>> GetForEmployeeAsync(long employeeId, CancellationToken cancellationToken = default)
        {
            return await _repository.GetQueryable()
                .AsNoTracking()
                .Where(x => x.EmployeeId == employeeId)
                .OrderBy(x => x.ManagementLevelId)
                .Select(x => new HcmWorkerManagerDto
                {
        ...
        public async Task<IEnumerable<HcmWorkerManagerDto>> GetAllAssignmentsAsync(CancellationToken cancellationToken = default)
        {
            return await _repository.GetQueryable()
                .AsNoTracking()
                .Select(x => new HcmWorkerManagerDto
        ...
        public async Task<IEnumerable<HcmWorkerManagerDto>> GetReportsAsync(long managerId, CancellationToken cancellationToken = default)
        {
            return await _repository.GetQueryable()
                .AsNoTracking()
        ...
        public async Task<IEnumerable<HcmWorkerManagerDto>> ReplaceForEmployeeAsync(long employeeId, IEnumerable<HcmWorkerManagerDto> rows, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var (levelId, managerId) in desired)
            {
                ...
                if (current == null)
                {
                    await _repository.AddAsync(new HcmWorkerManager
                    {
                    ...
                    if (current == employeeId)
                        throw new FluentValidation.ValidationException(new[]
                        {
                            new FluentValidation.Results.ValidationFailure(
                                nameof(HcmWorkerManagerDto.ManagerId),
