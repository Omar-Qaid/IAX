/**
 * Centralized, module-neutral Print Template Engine — Type Definitions
 *
 * These types describe the JSON schema for template documents, element trees,
 * field bindings, value formatting, and template metadata. They are intentionally
 * decoupled from any specific module (Workflow, Finance, etc.) so that every
 * module can reuse the same engine.
 */
...
// ---------------------------------------------------------------------------
// Field binding — extensible source types
// ---------------------------------------------------------------------------
...
export interface ReportDesignerSummary {
  templateId: number;
  /** Module-specific entity identifier (e.g. tableId of the entity). */
  refTableId: number;
  /** Module-specific entity identifier (e.g. recordId of the entity). */
  refRecId: number;
  code: string;
  name: string;
...
export interface PrintTemplate extends ReportDesignerSummary {
...
export interface PublishedPrintTemplate extends ReportDesignerSummary {
  templateVersionId: number;
  versionNo: number;
  document: PrintTemplateDocument;
}
