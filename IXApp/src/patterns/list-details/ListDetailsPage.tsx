...
import type { DetailSectionConfig, DetailValue, EnterpriseListDetailsConfig, ListDetailRecord, ListDetailsHeaderField } from './types';
...
function EnterpriseListDetailsPage<T extends ListDetailRecord>({ title, config, dialogs }: Omit<EnterpriseListDetailsProps<T>, 'variant'>): React.ReactElement {
  ...
  return <PageContainer sx={{ gap: 0, minHeight: { xs: 520, lg: 0 }, height: fullscreen ? 'calc(100vh - 5px)' : { xs: 'auto', lg: '100%' }, maxHeight: fullscreen ? 'calc(100vh - 5px)' : { lg: '100%' }, overflow: 'hidden', position: fullscreen ? 'fixed' : 'relative', inset: fullscreen ? '5px 0 0 5px' : undefined, zIndex: fullscreen ? 1300 : undefined, pr: { lg: `${d365.utilityRailWidth}px` }, bgcolor: d365.canvas, fontFamily: d365.fontFamily, color: d365.text, '& .MuiTypography-root, & .MuiButton-root, & .MuiInputBase-root, & input, & textarea': { fontFamily: `${d365.fontFamily} !important` } }}>
...
function RecordList<T extends ListDetailRecord>({ records, selectedId, editing, query, filterVisible, filterLabel, getPrimaryText, getSecondaryText, onQueryChange, onSelect }: {
  ...
  return <>{filterVisible && <Box sx={{ p: '12px 8px 9px' }}><TextField fullWidth size="small" placeholder={filterLabel} value={query} disabled={editing} onChange={(event) => onQueryChange(event.target.value)} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchIcon sx={{ fontSize: 14 }} /></InputAdornment> } }} sx={filterSx} /></Box>}<List disablePadding>{records.map((record) => <ListItemButton key={record.id} selected={record.id === selectedId} disabled={editing && record.id !== selectedId} onClick={() => onSelect(record)} sx={{ display: 'block', px: '10px', py: '8px', minHeight: 55, borderBottom: `1px solid ${d365.darkBorder}`, borderInlineStart: record.id === selectedId ? 4 : 0, borderInlineStartColor: d365.selectedBar, '&.Mui-selected, &.Mui-selected:hover': { bgcolor: d365.selectedRow } }}><Typography noWrap sx={{ fontSize: 15, fontWeight: 400, color: record.id === selectedId ? '#1746aa' : d365.text }}>{getPrimaryText(record)}</Typography>{getSecondaryText && <Typography sx={{ mt: '4px', fontSize: 10, color: record.id === selectedId ? '#1746aa' : d365.text }}>{getSecondaryText(record)}</Typography>}</ListItemButton>)}</List></>;
}
...
function RecordHeader<T>({ title, viewLabel, yesLabel, noLabel, record, fields, editing, maxWidth, onChange }: { title: string; viewLabel: string; yesLabel: string; noLabel: string; record: T; fields: ListDetailsHeaderField<T>[]; editing: boolean; maxWidth?: number; onChange: (id: string, value: DetailValue) => void }) {
  return <Box sx={{ px: 0, pt: '3px', pb: '10px', minHeight: 130, boxSizing: 'border-box' }}><Typography sx={{ fontSize: 11, lineHeight: 1.5 }}>{viewLabel}</Typography><Typography component="h1" sx={{ mt: '5px', mb: '14px', fontSize: d365.titleFontSize, lineHeight: 1.2, fontWeight: 600 }}>{title}</Typography><Box sx={{ display: 'grid', mt: '5px', width: '100%', maxWidth, gridTemplateColumns: { xs: '1fr 1fr', lg: fields.length === 2 ? '220px 80px' : `repeat(${Math.min(7, fields.length)},minmax(100px,1fr))` }, gap: '10px' }}>{fields.map((field) => {
    const value = field.getValue(record); const editable = editing && !field.disabled && field.type !== 'display';
    return <Box key={field.id}><Typography noWrap title={field.label} sx={{ mb: '2px', fontSize: d365.labelFontSize, lineHeight: 1.1, color: d365.text }}>{field.label}</Typography>{field.type === 'boolean' ? <Box sx={{ display: 'flex', alignItems: 'center', height: d365.controlHeight }}><Switch size="small" checked={Boolean(value)} disabled={!editable} onChange={(_, checked) => onChange(field.id, checked)} /><Typography sx={{ fontSize: d365.fontSize }}>{value ? yesLabel : noLabel}</Typography></Box> : editable && field.type === 'select' ? <TextField select value={value} onChange={(event) => onChange(field.id, event.target.value)} sx={headerEditFieldSx}>{(field.options ?? []).map((option) => <MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>)}</TextField> : editable ? <TextField type={field.type === 'number' ? 'number' : 'text'} value={value} onChange={(event) => onChange(field.id, field.type === 'number' ? Number(event.target.value) : event.target.value)} sx={headerEditFieldSx} /> : <HeaderViewField value={field.type === 'select' ? (field.options?.find((option) => option.value === String(value))?.label ?? value) : value} numeric={field.type === 'number'} />}</Box>;
  })}</Box></Box>;
}
...
const filterSx = { '& .MuiInputBase-root': { height: d365.controlHeight, borderRadius: d365.radius, fontSize: d365.fontSize }, '& .MuiInputAdornment-root': { mr: 0.25 } };
function HeaderViewField({ value, numeric = false }: { value: DetailValue; numeric?: boolean }): React.ReactElement {
  return <Box sx={{ minHeight: d365.controlHeight, display: 'flex', alignItems: 'center', justifyContent: numeric ? 'flex-end' : 'flex-start', border: `1px solid ${d365.darkBorder}`, borderRadius: d365.radius, px: 0.5, overflow: 'hidden', bgcolor: d365.surface }}><Typography noWrap sx={{ fontSize: d365.fontSize, color: '#1746aa' }}>{String(value ?? '')}</Typography></Box>;
}

const headerEditFieldSx = { width: '100%', mt: '5px', '& .MuiInputBase-root': { height: d365.controlHeight, borderRadius: d365.radius, fontSize: d365.fontSize }, '& .MuiInputBase-input': { px: 0.75, py: 0.25 } };
