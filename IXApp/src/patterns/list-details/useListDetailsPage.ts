...
export function useListDetailsPage<T extends ListDetailRecord>(
  ...
  const [localRecords, setLocalRecords] = useState<T[]>(
    source.type === 'remote' ? (source.initialRecords ?? []) : source.records
  );
  const [loading, setLoading] = useState(
    source.type === 'remote' || (source.type === 'controlled' && Boolean(source.loading))
  );
  const [error, setError] = useState<string | null>(
    source.type === 'controlled' ? (source.error ?? null) : null
  );
  ...
  const filterStorageId =
    (config.filterStorageKey ??
      ...
      config.recordTableName ??
      remoteSourceKey) ||
    globalThis.location?.pathname ||
    'default';
  const [query, setQuery] = useLocalStorage<string>(
    `ixapp.list-details.query.${filterStorageId}`,
    config.initialQuery ?? ''
  );
  ...
  const [informationPanelOpen, setInformationPanelOpen] = useState(
    config.informationOpenOnLoad ?? false
  );
  ...
  useEffect(() => {
    ...
    setSelectedId((current) =>
      loaded.some((record) => record.id === current) ? current : (loaded[0]?.id ?? null)
    );
  ...
  useEffect(() => {
    ...
    if (editing) return;
    setSelectedId((current) =>
      controlledRecords?.some((record) => record.id === current)
        ? current
        : (controlledRecords?.[0]?.id ?? null)
    );
    setDraft(
      (current) =>
        controlledRecords?.find((record) => record.id === current?.id) ??
        controlledRecords?.[0] ??
        null
    );
    ...
    return records.filter((record) => {
      const matchesQuery =
        ...
        (config.matchesSearch
          ? config.matchesSearch(record, normalized)
          : config
      ...
      const matchesAdvanced =
        ...
        advancedFilters.every((condition) => {
          const field = config.advancedFilter?.fields?.find(
            (candidate) => candidate.id === condition.field
          );
          const value = field?.getValue(record) ?? config.advancedFilter?.getValue?.(record);
          return value !== undefined
            ? matchesEnterpriseFilter(value, condition)
            : config.advancedFilter!.matches(record, condition.value);
        });
  ...
  const cancel = () => {
    if (isNew) setSelectedId(records[0]?.id ?? null);
    setDraft(isNew ? (records[0] ?? null) : selected);
    setEditing(false);
    setIsNew(false);
    setValidationErrors({});
  };
  ...
  const changeValue = (name: string, value: DetailValue) => {
    ...
    setDraft((current) =>
      current ? config.setValues(current, { ...config.getValues(current), [name]: value }) : current
    );
  };
  const changeHeader = (fieldId: string, value: DetailValue) => {
    setValidationErrors((current) => {
      const next = { ...current };
      delete next[fieldId];
      return next;
    ...
    setDraft((current) => {
      const field = config.headerFields.find((candidate) => candidate.id === fieldId);
      return current && field ? field.setValue(current, value) : current;
    });
  };
  return {
    records,
    ...
    validationErrors,
    visibleRecords,
    numberSequenceMetadata: numberSequenceQuery.data ?? null,
    numberSequenceLoading: numberSequenceQuery.isFetching,
    setQuery,
    setDraftAdvancedFilters,
    applyAdvancedFilter: () =>
      setAdvancedFilters(draftAdvancedFilters.filter((condition) => condition.value.trim())),
    resetAdvancedFilter: () => {
      setDraftAdvancedFilters([createEnterpriseFilterCondition(defaultAdvancedField)]);
      setAdvancedFilters([]);
    },
    toggleFilter: () =>
      config.advancedFilter
    ...
    toggleInformation: () => setInformationPanelOpen((open) => !open),
    choose,
    startEdit,
    startNew,
    save,
