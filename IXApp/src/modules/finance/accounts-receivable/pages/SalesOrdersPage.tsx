...
export function SalesOrdersPage(): React.ReactElement {
  ...
  const openOrder = (order: SalesOrder) => navigate(encodeURIComponent(order.id));
  return (
    <SimpleListPage
      title={t('pages.salesOrders.title')}
      subtitle={t('pages.salesOrders.subtitle')}
      dataSource={{ type: 'static', rows: MOCK_SALES_ORDERS }}
      columns={columns}
      dataGridProps={{
        storageKey: 'accounts-receivable.sales-orders',
