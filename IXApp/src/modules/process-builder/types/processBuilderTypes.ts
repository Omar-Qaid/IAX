...
export interface BuilderVariable { id: string; code: string; name: string; nameAR: string; description: string; descriptionAR: string; dataType: BuilderDataType; sortOrder: number; required: boolean; active: boolean; scope: 'process' | 'step' | 'activity' | 'global'; defaultValue: string }
...
export interface BuilderActivity { id: string; code: string; name: string; nameAR: string; type: BuilderActivityType; performer: string; assignmentMode: 'any' | 'all' | 'round-robin'; active: boolean; required: boolean; mandatoryDocs: boolean; autoPassEnabled: boolean; autoPassingHours: number; controls: BuilderControl[]; actions: BuilderActivityAction[]; validations: BuilderValidation[]; condition: BuilderCondition | null; config: { apiMethod: 'GET' | 'POST' | 'PUT' | 'DELETE'; apiUrl: string; notifyEmails: string } }
export interface BuilderStep { id: string; code: string; name: string; nameAR: string; order: number; score: number; autoPassingHours: number; allMandatory: boolean; active: boolean; systemField: boolean; condition: BuilderCondition | null; activities: BuilderActivity[] }
