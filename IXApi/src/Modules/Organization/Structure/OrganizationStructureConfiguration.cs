...
public sealed class OrganizationHierarchyNodeConfiguration : IEntityTypeConfiguration<OrganizationHierarchyNode>
{
    public void Configure(EntityTypeBuilder<OrganizationHierarchyNode> b)
    {
        ...
        b.HasOne(x => x.Hierarchy).WithMany().HasForeignKey(x => x.HierarchyId).OnDelete(DeleteBehavior.Restrict);
        b.HasOne(x => x.OrganizationUnit).WithMany().HasForeignKey(x => x.OrganizationUnitId).OnDelete(DeleteBehavior.Restrict);
        b.HasOne(x => x.ParentNode).WithMany().HasForeignKey(x => x.ParentNodeId).OnDelete(DeleteBehavior.Restrict);
