...
export function FilterPopover<T>({
  ...
  React.useEffect(() => {
    if (anchorEl && column) {
      ...
      setLocalFilter(
        existing || { field: column.field as string, operator: 'contains', value: '' }
      );
      setTempValue('');
    }
  ...
  return (
    <>
      <Popover
        ...
        slotProps={{
          paper: {
            sx: { width: 300, p: 2, borderRadius: 1, boxShadow: '0 4px 20px rgba(0,0,0,0.15)' },
          },
        }}
      >
            ...
            {localFilter.value.map((val: string) => (
              <Chip
                ...
                onDelete={() =>
                  setLocalFilter((f) => ({
                    ...f,
                    value: (f.value as string[]).filter((v) => v !== val),
                  }))
                }
      ...
      >
        {GRID_FILTER_OPERATORS.map((op: { value: string; label: string }) => (
          <MenuItem
            key={op.value}
            onClick={() => {
              setLocalFilter((f) => ({ ...f, operator: op.value as FilterModel['operator'] }));
              setOperatorAnchor(null);
            }}
          >
            <ListItemText
              primary={getOperatorLabel(op.value, t)}
              slotProps={{ primary: { sx: { fontSize: '0.85rem' } } }}
            />
          </MenuItem>
        ))}
