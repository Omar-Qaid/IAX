...
const toBuilderVariable = (
  ...
  code: variable.code ?? '',
  name: variable.name ?? '',
  nameAlias: variable.nameAlias ?? null,
...
export async function loadProcessBuilder(processId: number, signal?: AbortSignal): Promise<ProcessBuilderDocument> {
  const [
    process,
    variables,
    steps,
    ...
    wfStepApi.list(signal, processId),
    wfActivityApi.list(signal, processId),
    wfActivityTypeApi.list(signal),
    wfActivityControlApi.list(signal, processId),
    wfActivityControlValidationApi.list(signal, processId),
    wfActivityControlOptionApi.list(signal, processId),
    wfRequestControlApi.list(signal, processId),
    wfControlApi.list(signal),
    wfRequestControlValidationApi.list(signal, processId),
  ...
  const builderSteps: BuilderStep[] = processSteps.map((step) => ({
    ...
    code: step.code ?? '',
    name: step.name ?? '',
  nameAlias: step.nameAlias ?? null,
    order: step.sortOrder,
    score: step.score,
    allMandatory: step.mustCompleteAll,
    active: step.isActive,
    systemField: step.isSystemDefined,
    condition: null,
    activities: activities
      ...
      .map<BuilderActivity>((activity) => ({
        ...
        code: activity.code ?? '',
        name: activity.name ?? '',
  nameAlias: activity.nameAlias ?? null,
        ...
        isWhatsAppNotificationEnabled: activity.isWhatsAppNotificationEnabled ?? false,
        canViewPreviousSteps: activity.canViewPreviousSteps ?? false,
        canViewPreviousDocuments: activity.canViewPreviousDocuments ?? false,
  ...
  return {
    ...
    code: process.code ?? '',
    name: process.name ?? '',
  nameAlias: process.nameAlias ?? null,
    description: process.description ?? '',
    categoryId: String(process.categoryId || ''),
    priorityId: String(process.priorityId || ''),
    processType: String(process.processTypeId || ''),
    score: process.score,
    repeatIntervalHours: process.repeatIntervalHours ?? 0,
    canRepeat: process.isRepeatable,
    mandatoryDocs: process.mandatoryDocuments,
...
export async function saveProcessBuilder(
  ...
  const record: WfProcessRecord = {
    ...
    name: document.name.trim(),
      ...(document.nameAlias !== undefined ? { nameAlias: document.nameAlias?.trim() || null } : {}),
    description: document.description.trim() || null,
    categoryId,
    priorityId,
    processTypeId,
    score: document.score,
    repeatIntervalHours: document.repeatIntervalHours ?? 0,
    isRepeatable: document.canRepeat,
    mandatoryDocuments: document.mandatoryDocs,
  ...
  for (const step of document.steps) {
    ...
    const stepRecord = {
      ...
      name: step.name,
      ...(step.nameAlias !== undefined ? { nameAlias: step.nameAlias?.trim() || null } : {}),
      processId: persisted.recId,
      sortOrder: step.order,
      score: step.score,
      mustCompleteAll: step.allMandatory,
      isSystemDefined: step.systemField,
  ...
  for (const step of document.steps) {
    ...
    for (const activity of step.activities) {
      ...
      const activityRecord = {
        ...
        name: activity.name,
      ...(activity.nameAlias !== undefined ? { nameAlias: activity.nameAlias?.trim() || null } : {}),
        stepId,
        ...
        isWhatsAppNotificationEnabled: activity.isWhatsAppNotificationEnabled ?? current?.isWhatsAppNotificationEnabled ?? false,
        canViewPreviousSteps: activity.canViewPreviousSteps ?? current?.canViewPreviousSteps ?? false,
        canViewPreviousDocuments: activity.canViewPreviousDocuments ?? current?.canViewPreviousDocuments ?? false,
...
export async function saveProcessVariables(
  ...
  for (const variable of [...document.variables].sort((a, b) => a.sortOrder - b.sortOrder)) {
    ...
    const variableRecord = {
      ...
      ...(variable.nameAlias !== undefined ? { nameAlias: variable.nameAlias?.trim() || null } : {}),
...
export async function saveProcessRequestControls(
  ...
  for (const { control, controlType } of resolved) {
    ...
    const record = {
      ...
      uniqueKey: control.uniqueKey,
      usedAsCriteria: control.usedAsCriteria,
      canFilter: control.canFilter,
      canGroup: control.canGroup,
      canSort: control.canSort,
