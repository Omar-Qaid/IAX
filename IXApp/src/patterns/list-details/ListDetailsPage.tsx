...
function EnterpriseListDetailsPage<T extends ListDetailRecord>({ title, config, dialogs }: Omit<EnterpriseListDetailsProps<T>, 'variant'>): React.ReactElement {
  ...
  const listPane = config.presentation?.mode === 'grid' && config.presentation.columns
    ? <Box sx={{ height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column' }}>{state.filterVisible && <Box sx={{ p: 1 }}><TextField fullWidth size="small" placeholder={labels.filter} value={state.query} disabled={state.editing} onChange={(event) => state.setQuery(event.target.value)} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchIcon sx={{ fontSize: 16 }} /></InputAdornment> } }} sx={filterSx} /></Box>}<Box sx={{ flex: 1, minHeight: 0 }}><DataGrid rows={state.visibleRecords} columns={config.presentation.columns} loading={state.loading} height="100%" hideToolbar hideFooter hideSidebar hideFilterRow selectionMode="single" selectedIds={state.selectedId ? [state.selectedId] : []} onRowClick={state.choose} storageKey={config.presentation.storageKey} /></Box></Box>
  ...
  return <PageContainer sx={{ gap: 0.5, minHeight: { xs: 520, lg: 0 }, height: { xs: 'auto', lg: '100%' }, maxHeight: { lg: '100%' }, overflow: { lg: 'hidden' }, position: 'relative', pr: { lg: '38px' }, bgcolor: '#faf9f8' }}>
    ...
    {state.error ? <ErrorState message={state.error} onRetry={state.refresh} /> : state.loading && !state.records.length ? <LoadingState /> : record ? <ListDetailsLayout editing={state.editing} values={config.getValues(record)} yesLabel={labels.yes} noLabel={labels.no} sections={config.sections} onChange={state.changeValue} listWidth={config.presentation?.listWidth}
      listPane={listPane}
      header={<><RecordHeader title={title} viewLabel={labels.view} record={record} fields={config.headerFields} editing={state.editing} onChange={state.changeHeader} />{Object.keys(state.validationErrors).length > 0 && <Alert severity="error" sx={{ mb: 1 }}><Typography sx={{ fontWeight: 600, fontSize: '0.75rem' }}>{config.validationTitle ?? t('validation.correctErrors', 'Please correct the validation errors.')}</Typography>{Object.entries(state.validationErrors).map(([field, message]) => <Typography key={field} sx={{ fontSize: '0.6875rem' }}>{message}</Typography>)}</Alert>}</>}
    /> : <EmptyState title="No records" />}
    {state.filterPanelOpen && config.advancedFilter && <Box sx={sidePanelSx}><EnterpriseFilterPanel title={config.advancedFilter.title ?? t('filters.title')} addLabel={config.advancedFilter.addLabel ?? t('actions.add')} fieldLabel={config.advancedFilter.fieldLabel} operatorLabel={config.advancedFilter.operatorLabel ?? t('filters.contains')} value={state.draftAdvancedFilter} applyLabel={config.advancedFilter.applyLabel ?? t('actions.apply')} resetLabel={config.advancedFilter.resetLabel ?? t('actions.reset')} onValueChange={state.setDraftAdvancedFilter} onApply={state.applyAdvancedFilter} onReset={state.resetAdvancedFilter} onRemove={state.resetAdvancedFilter} /></Box>}
