...
export function ReportThumbnailPanel({ open, currentPage, totalPages, onPageChange, renderThumbnail }: ReportThumbnailPanelProps): React.ReactElement {
  ...
  return (
    <Paper
      ...
      sx={{ minWidth: 0, display: 'grid', gridTemplateRows: '31px minmax(0, 1fr)', overflow: 'hidden', bgcolor: '#d7d8dc', borderRight: '1px solid #adb2ba' }}
    >
      {open ? <>
        <Stack direction="row" spacing={0.1} sx={{ alignItems: 'center', px: 0.5, bgcolor: '#eeeeef', borderBottom: '1px solid #b8bdc4' }}>
          <IconButton size="small" color="primary" aria-label="Page thumbnails"><GridViewOutlined sx={{ fontSize: 17 }} /></IconButton>
          <IconButton size="small" aria-label="Report outline"><ListOutlined sx={{ fontSize: 17 }} /></IconButton>
          <IconButton size="small" aria-label="Report attachments"><AttachFileOutlined sx={{ fontSize: 17 }} /></IconButton>
          <IconButton size="small" aria-label="Report layers"><LayersOutlined sx={{ fontSize: 17 }} /></IconButton>
        </Stack>
        <Box ref={scrollRef} sx={{ overflowY: 'auto', overflowX: 'hidden' }}>
          <Box sx={{ position: 'relative', height: virtualizer.getTotalSize() }}>
            {virtualizer.getVirtualItems().map((item) => {
              ...
              return <Box key={page} onClick={() => onPageChange(page)} sx={{ position: 'absolute', top: 0, left: 0, width: '100%', height: item.size, transform: `translateY(${item.start}px)`, cursor: 'pointer', pt: 1.5 }}>
