...
- Notification delivery is not an exactly-once outbox: existing option notifications can still dispatch before the enclosing transaction commits. The legacy scheduled-notification worker still lacks a durable claim/lease and account/company contract. Completing checkpoint E requires extending that existing Communication worker, not creating a parallel notification subsystem. Scheduled requests inherit this current limitation.
