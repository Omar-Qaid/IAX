...
import {
  Menu, MenuItem, ListItemIcon, ListItemText, Divider, Box
} from '@mui/material';
import {
  ArrowUpward, ArrowDownward, PushPin as PushPinIcon,
...
export function HeaderMenu<T>({
  anchorEl, onClose, activeColumn, initialColumns, setColumns, onSort, onResetColumns, onOpenChooseColumns
}: HeaderMenuProps<T>) {
  ...
  const handlePin = (position: 'left' | 'right' | null) => {
    if (activeColumn) {
      setColumns(prev => prev.map(c =>
        c.field === activeColumn.field ? { ...c, pinned: position } : c
      ));
    }
    onClose();
  ...
  return (
    <>
      <Menu
        ...
        onClose={() => { onClose(); setPinMenuAnchor(null); }}
      ...
      >
        ...
        <MenuItem
          ...
          sx={{ display: 'flex', justifyContent: 'space-between', bgcolor: pinMenuAnchor ? 'action.hover' : 'transparent' }}
        >
