...
export const createProcessBuilderDocument = (builderId = 'new'): ProcessBuilderDocument => ({
  id: builderId, code: builderId === 'new' ? 'PB-DRAFT' : `PB-${builderId}`, name: 'New approval process', description: 'Configure the request, steps, activities, and routing conditions.', categoryId: '', priorityId: '', processType: 'Workflow Process', score: 100, canRepeat: false, mandatoryDocs: false, active: true,
  variables: [{ id: id(), code: 'AMOUNT', name: 'Request amount', description: '', dataType: 'number', sortOrder: 10, required: true, active: true, scope: 'process', defaultValue: '0' }],
  ...
  steps: [{ id: id(), code: 'STEP-00001', name: 'Manager review', order: 1, score: 0, autoPassingHours: 0, allMandatory: true, active: true, systemField: false, condition: null, activities: [] }], transitions: [],
});
...
export const useProcessBuilderStore = create<State>((set) => {
  ...
  return {
    ...
    addVariable: () => change((d) => ({ ...d, variables: [...d.variables, { id: id(), code: `VAR${d.variables.length + 1}`, name: 'New variable', description: '', dataType: 'text', sortOrder: (d.variables.length + 1) * 10, required: false, active: true, scope: 'process', defaultValue: '' }] })),
    ...
    addStep: () => change((d) => ({ ...d, steps: [...d.steps, { id: id(), code: `STEP-${String(d.steps.length + 1).padStart(5, '0')}`, name: `Step ${d.steps.length + 1}`, order: d.steps.length + 1, score: 0, autoPassingHours: 0, allMandatory: false, active: true, systemField: false, condition: null, activities: [] }] })),
