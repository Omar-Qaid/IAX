...
import { WorkflowOfficialFormViewer } from '@modules/workflow/pages/WorkflowOfficialFormPage';

vi.mock('@modules/workflow/print-templates/api/printTemplateApi', () => ({
  printTemplateApi: { getPublishedForRequest: vi.fn().mockResolvedValue({ templateId: 17, processId: 7, code: 'OFFICIAL', name: 'Official request form', document: { schemaVersion: 1, language: 'en', direction: 'ltr', page: { size: 'A4', orientation: 'portrait', margins: { top: 15, right: 15, bottom: 15, left: 15 } }, header: [{ id: 'title', type: 'text', value: 'Approved official layout' }], sections: [{ id: 'amount', type: 'field', label: 'Total sales', binding: { sourceType: 'requestControl', requestControlId: 2101 } }], footer: [], missingFieldBehavior: 'empty' } }) },
}));
...
vi.mock('@shared/components/printout/reportCompany', () => ({
  fetchPrintoutCompany: vi.fn().mockResolvedValue({ name: 'USMF', companyCode: 'HBMC' }),
  toPrintoutCompany: (_entity: unknown, code: string) => ({ name: code || 'Company', companyCode: code }),
}));
