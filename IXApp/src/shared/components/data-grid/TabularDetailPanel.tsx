...
import type { ColumnDef, DataGridHandle } from './types';
import { d365 } from '@shared/constants/enterpriseUiTokens';

export interface TabularDetailAction {
  id: string;
  label: string;
  ariaLabel?: string;
  onClick?: () => void;
...
export function TabularDetailPanel<T extends { id: string }>({
  rows,
  columns,
  addLabel,
  removeLabel,
  ...
  onNewRow,
  onEditingChange,
}: TabularDetailPanelProps<T>): React.ReactElement {
  return (
    <Box sx={{ minWidth: 0 }}>
      ...
      <DataGrid
        ...
        hideFooter
        hideSidebar
        hideFilterRow={!showFilterRow}
        selectionMode="single"
        selectedIds={selectedIds}
        onSelectionChange={(ids) => onSelectionChange(ids as (string | number)[])}
        storageKey={storageKey}
        rowHeight={rowHeight}
        headerHeight={d365.gridHeaderHeight}
