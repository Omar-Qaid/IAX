...
/// <summary>
/// Seeds the Payment Request (طلب الصرف) Process Builder definition translated
/// from the supplied legacy SQL into the current normalized workflow schema.
/// Runtime requests and history are intentionally not seeded.
/// </summary>
public sealed class WfProcessSeedData : ISeeder
{
    ...
    private static WfRequestControl RequestControl(long processId, byte controlId, string code,
        string name, string description, byte order, string owner) => new()
    {
        ProcessId = processId, ControlId = controlId, Code = code, Name = name,
        Description = description, SortOrder = order, ValidationRules = RequiredRule,
