...
import { hcmNationalityApi, type HcmReferenceRecord } from '../api/hcmReferenceApi';
import { useNavigate } from 'react-router-dom';

const queryKey = ['simple-list', 'hcm-nationalities'] as const;

export function HcmNationalityPage(): React.ReactElement {
  const { t, currentLanguage } = useAppTranslation();
  const { notifyError, notifySuccess } = useNotifications();
  const navigate = useNavigate();

  const columns = useMemo<ColumnDef<HcmReferenceRecord>[]>(() => [
    { field: 'code', headerName: 'hcmNationalities.fields.code', width: 140, pinned: 'left', editable: true },
  ...
  const config: EnterpriseListConfig<HcmReferenceRecord> = {
    ...
    recordTableName: 'HcmNationality',
    ...
    crud: {
      ...
      onDelete: async (rows) => {
        try {
          await Promise.all(rows.map((row) => hcmNationalityApi.delete(row)));
          await refresh();
    ...
    advancedFilter: {
      ...
      fieldLabel: t('hcmNationalities.fields.code'),
  ...
  return (
    <SimpleListPage
      title={t('hcmNationalities.title')}
      enterpriseConfig={config}
      dataSource={{
        type: 'remote',
        key: 'hcm-nationalities',
        load: (signal) => hcmNationalityApi.list(signal),
      }}
      columns={columns}
      dataGridProps={{
        storageKey: 'organization.hcm-nationalities.reference-view',
        masterForm: true,
        hideSidebar: false,
        ...
        onRowSave: async (values, isNew) => {
          ...
          if (!record.code.trim()) throw new Error(t('validation.required', { field: t('hcmNationalities.fields.code') }));
          if (!record.name.trim()) throw new Error(t('validation.required', { field: t('fields.name') }));
          if (isNew || record.recId === 0) {
            await hcmNationalityApi.create(record);
          } else {
            await hcmNationalityApi.update(record);
          }
          await refresh();
