...
function RecordList<T extends ListDetailRecord>({ records, selectedId, editing, query, filterVisible, filterLabel, getPrimaryText, getSecondaryText, onQueryChange, onSelect }: {
  ...
  return <>{filterVisible && <Box sx={{ p: '14px 10px 8px' }}><TextField fullWidth size="small" placeholder={filterLabel} value={query} disabled={editing} onChange={(event) => onQueryChange(event.target.value)} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchIcon sx={{ fontSize: 14 }} /></InputAdornment> } }} sx={filterSx} /></Box>}<List disablePadding>{records.map((record) => <ListItemButton key={record.id} selected={record.id === selectedId} disabled={editing && record.id !== selectedId} onClick={() => onSelect(record)} sx={{ display: 'block', px: '10px', py: '10px', minHeight: 67, borderBottom: `1px solid ${d365.darkBorder}`, borderInlineStart: record.id === selectedId ? 4 : 0, borderInlineStartColor: d365.selectedBar, '&.Mui-selected, &.Mui-selected:hover': { bgcolor: d365.selectedRow } }}><Typography noWrap sx={{ fontSize: 15, lineHeight: 1.3, fontWeight: 400, color: record.id === selectedId ? '#1746aa' : d365.text }}>{getPrimaryText(record)}</Typography>{getSecondaryText && <Typography sx={{ mt: '6px', fontSize: 10, lineHeight: 1.2, color: record.id === selectedId ? '#1746aa' : d365.text }}>{getSecondaryText(record)}</Typography>}</ListItemButton>)}</List></>;
}
...
function RecordHeader<T>({ title, viewLabel, yesLabel, noLabel, record, fields, editing, maxWidth, onChange }: { title: string; viewLabel: string; yesLabel: string; noLabel: string; record: T; fields: ListDetailsHeaderField<T>[]; editing: boolean; maxWidth?: number; onChange: (id: string, value: DetailValue) => void }) {
  return <Box sx={{ px: 0, pt: '3px', pb: '3px', minHeight: 130, boxSizing: 'border-box' }}><Typography sx={{ fontSize: 11, lineHeight: 1.5 }}>{viewLabel}</Typography><Typography component="h1" sx={{ mt: '5px', mb: '21px', fontSize: d365.titleFontSize, lineHeight: 1.2, fontWeight: 600 }}>{title}</Typography><Box sx={{ display: 'grid', mt: '5px', width: '100%', maxWidth, gridTemplateColumns: { xs: '1fr 1fr', lg: fields.length === 2 ? '220px 80px' : `repeat(${Math.min(7, fields.length)},minmax(100px,1fr))` }, gap: '10px' }}>{fields.map((field) => {
...
const filterSx = { '& .MuiInputBase-root': { height: 34, borderRadius: d365.radius, fontSize: d365.fontSize }, '& .MuiInputAdornment-root': { mr: 0.25 } };
function HeaderViewField({ value, numeric = false }: { value: DetailValue; numeric?: boolean }): React.ReactElement {
  return <Box sx={{ minHeight: d365.controlHeight, display: 'flex', alignItems: 'center', justifyContent: numeric ? 'flex-end' : 'flex-start', borderBottom: `1px solid ${d365.darkBorder}`, px: 0.5, overflow: 'hidden', bgcolor: 'transparent' }}><Typography noWrap sx={{ fontSize: d365.fontSize, color: '#315efb' }}>{String(value ?? '')}</Typography></Box>;
}
