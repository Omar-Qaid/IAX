...
describe('DynamicControlRenderer', () => {
  it('edits table rows using configured process-builder columns', async () => {
    ...
    await user.type(screen.getByRole('textbox', { name: 'Item 1' }), 'Taxi');
    await user.type(screen.getByRole('textbox', { name: 'Amount 1' }), '75');

    expect(JSON.parse(changes.at(-1) ?? '[]')).toEqual([{ item: 'Taxi', amount: '75' }]);
    await user.click(screen.getByRole('button', { name: 'Delete 1' }));
    expect(JSON.parse(changes.at(-1) ?? '[]')).toEqual([]);
  });

  it('renders a configured label control as display-only content', () => {
    render(
      <DynamicControlRenderer
        control={{ label: 'Applicant information', labelColor: '#2457a7', controlType: 'label' }}
        value=""
        onChange={() => undefined}
      />
    );
    expect(screen.getByRole('note')).toHaveTextContent('Applicant information');
    expect(screen.getByText('Applicant information')).toHaveStyle({
      color: '#2457a7',
      fontWeight: '800',
    });
  ...
  it('reveals dependent controls directly beneath the selected option', async () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <DynamicForm processId={591} showActions={false} />
      </QueryClientProvider>
    );

    await screen.findByRole('radio', { name: /Salary deduction/ });
    expect(
      Array.from(screen.getByTestId('dynamic-form-grid').querySelectorAll('[data-control-id]')).map(
        (element) => element.getAttribute('data-control-id')
      )
    ).toEqual(['10', '30', '40']);
    expect(screen.queryByLabelText('Amount')).not.toBeInTheDocument();
    await userEvent.click(screen.getByRole('radio', { name: /Salary deduction/ }));

    const dependencies = await screen.findByRole('group', {
      name: 'Salary deduction additional fields',
    });
    await waitFor(() => expect(within(dependencies).getByLabelText('Amount')).toBeVisible());
    expect(
      within(dependencies).getByRole('button', { name: 'Attach Supporting document' })
    ).toBeVisible();

    await userEvent.click(screen.getByRole('radio', { name: /Termination/ }));
    const fullRowDependencies = await screen.findByRole('group', {
      name: 'Termination additional fields',
    });
  ...
  it('supports canonical radio and calendar metadata types', () => {
    ...
    rerender(
      <DynamicControlRenderer
        control={{ label: 'Travel date', controlType: 'calendar' }}
        value="2026-08-20"
        onChange={() => undefined}
      />
    );
  ...
  it('renders a metadata-validated drag and drop file control', () => {
    ...
    const { container, rerender } = render(
      <DynamicControlRenderer control={fileControl} value="" onChange={onChange} />
    );

    expect(screen.getByRole('button', { name: 'Upload Documents' })).toHaveTextContent(
      'Drop documents here or Browse'
    );
    expect(screen.getByRole('button', { name: 'Upload Documents' })).toHaveStyle({
      height: '30px',
    });
    ...
    rerender(
      <DynamicControlRenderer
        control={fileControl}
        value={'[{"n":"document.pdf","s":7,"t":"application/pdf"}]'}
        onChange={onChange}
      />
    );
  ...
  it('renders compact attachments as wrapping chips with preview, remove, and add actions', async () => {
    ...
    render(
      <DynamicControlRenderer
        ...
        value={
          '[{"n":"1001.jpg","s":1024,"t":"image/jpeg"},{"n":"a-very-long-supporting-document-name.pdf","s":2048,"t":"application/pdf"}]'
        }
        onChange={onChange}
      />
    );
    ...
    expect(
      within(attachments).getByRole('button', {
        name: 'Preview a-very-long-supporting-document-name.pdf',
      })
    ).toHaveStyle({ maxWidth: '160px' });
    ...
    expect(onChange).toHaveBeenLastCalledWith(
      expect.stringContaining('a-very-long-supporting-document-name.pdf')
    );
  });
  ...
  it('shows image thumbnails and opens a larger preview popup', async () => {
    ...
    const { container, rerender, unmount } = render(
      <DynamicControlRenderer control={control} value="" onChange={onChange} />
    );
    ...
    rerender(
      <DynamicControlRenderer
        control={control}
        value={'[{"n":"photo.png","s":5,"t":"image/png"}]'}
        onChange={onChange}
      />
    );

    expect(
      screen.getByRole('button', { name: 'Preview photo.png' }).querySelector('img')
    ...
    expect(screen.getByAltText('Preview of photo.png')).toHaveAttribute(
      'src',
      'blob:photo-preview'
    );
  ...
  it('renders signature and location controls from canonical metadata types', async () => {
    ...
    const { rerender } = render(
      <DynamicControlRenderer
        control={{ label: 'E-Signature', controlType: 'signature' }}
        value=""
        onChange={() => undefined}
      />
    );
    ...
    rerender(
      <DynamicControlRenderer
        control={{ label: 'Work location', controlType: 'location' }}
        value=""
        onChange={() => undefined}
      />
    );
  ...
  it('renders a stored image signature without exposing its encoded value', () => {
    ...
    expect(screen.getByRole('img', { name: 'E-Signature signature' })).toHaveAttribute(
      'src',
      signature
    );
  ...
  it('offers edit and remove actions over a saved location', () => {
    ...
    render(
      <DynamicControlRenderer
        control={{ label: 'Work location', controlType: 'location' }}
        value={'{"address":"Jeddah","latitude":21.5433,"longitude":39.1728}'}
        onChange={onChange}
      />
    );
