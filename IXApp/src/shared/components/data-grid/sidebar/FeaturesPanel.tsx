...
import {
  ...
  DensityLarge as ComfortableIcon,
...
export function FeaturesPanel<T>({
  columns, selectionMode, setSelectionMode, onExport, onServerImport, onDownloadTemplate,
  onAutosizeAll, onAutosizeColumn, onUnAutosizeColumn, onResetColumns, isAutosized,
  rowHeight, setRowHeight, showColumnBorders, setShowColumnBorders, showCellBorders, setShowCellBorders,
  setActiveTab
  ...
  const handleFileChosen = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImporting(true);
    try { await onServerImport(file); } finally { setImporting(false); }
  };
    ...
    <Box sx={{ flexGrow: 1, minHeight: 0, overflow: 'auto', p: 1.5 }}>
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
          ...
          <Typography variant="subtitle2" sx={{ fontSize: '0.85rem', fontWeight: 600 }}>{t('grid.themes')}</Typography>
        </Box>

        <Box sx={{ mb: 2 }}>
          <Box sx={{ pl: 3.5, display: 'flex', flexDirection: 'column', gap: 1 }}>
            {[
              { label: t('grid.compact_density'), value: 28, icon: <CompactIcon sx={{ fontSize: 18 }} /> },
              { label: t('grid.standard_density'), value: 36, icon: <StandardIcon sx={{ fontSize: 18 }} /> },
              { label: t('grid.comfortable_density'), value: 48, icon: <ComfortableIcon sx={{ fontSize: 18 }} /> },
            ].map(d => (
              <Box
                key={d.label}
              ...
              >
                ...
                <Typography variant="body2" sx={{ fontSize: '0.85rem', fontWeight: rowHeight === d.value ? 600 : 400 }}>
                  {d.label}
                </Typography>
              </Box>
            ))}
        ...
        <Box sx={{ pl: 3.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Typography variant="body2" sx={{ fontSize: '0.75rem' }}>{t('grid.show_column_borders')}</Typography>
            </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Typography variant="body2" sx={{ fontSize: '0.75rem' }}>{t('grid.show_cell_borders')}</Typography>
            </Box>
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
          ...
          <Typography variant="subtitle2" sx={{ fontSize: '0.85rem', fontWeight: 600 }}>{t('grid.row_selection')}</Typography>
        </Box>
        <Box sx={{ pl: 3.5, display: 'flex', flexDirection: 'column', gap: 0.75 }}>
          {(['single', 'multiple'] as SelectionMode[]).map(mode => (
            ...
            >
              <Box sx={{ color: selectionMode === mode ? 'primary.main' : 'text.secondary', display: 'flex' }}>
                {selectionMode === mode ? <RadioCheckedIcon sx={{ fontSize: 18 }} /> : <RadioUncheckedIcon sx={{ fontSize: 18 }} />}
              </Box>
              <Box>
                <Typography variant="body2" sx={{ fontSize: '0.85rem', fontWeight: selectionMode === mode ? 600 : 400, textTransform: 'capitalize' }}>
                  {mode === 'single' ? t('grid.single_selection') : t('grid.multiple_selection')}
                </Typography>
                {mode === 'single' && <Typography variant="caption" sx={{ color: 'text.disabled', display: 'block', fontSize: '0.7rem' }}>{t('grid.single_selection_desc')}</Typography>}
                {mode === 'multiple' && <Typography variant="caption" sx={{ color: 'text.disabled', display: 'block', fontSize: '0.7rem' }}>{t('grid.multiple_selection_desc')}</Typography>}
              </Box>
            </Box>
          ))}
        </Box>
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
          ...
          <Typography variant="subtitle2" sx={{ fontSize: '0.85rem', fontWeight: 600 }}>{t('grid.column_sizing')}</Typography>
        </Box>
        <Box sx={{ pl: 3.5, display: 'flex', flexDirection: 'column', gap: 1 }}>
          <Button
            size="small"
            ...
            sx={{ 
              ...
              fontSize: '0.75rem', 
              mb: 1,
              ...(isAutosized && { bgcolor: 'primary.main', '&:hover': { bgcolor: 'primary.dark' } })
            }}
          >
            ...
            {columns.filter(c => !c.hidden).map(col => (
              <Box
                ...
                sx={{
                  ...
                  p: '2px 4px', borderRadius: 0.5, '&:hover': { bgcolor: '#f0f0f0' }
                }}
              >
                <Typography variant="body2" sx={{ fontSize: '0.75rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flexGrow: 1 }}>
                  {col.headerName}
                </Typography>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        <Button
          ...
          sx={{ textTransform: 'none', fontSize: '0.8rem', color: 'error.main', borderColor: 'error.light', '&:hover': { borderColor: 'error.main', bgcolor: 'error.50' } }}
        >
          {t('grid.reset_layout')}
        </Button>
      </Box>
