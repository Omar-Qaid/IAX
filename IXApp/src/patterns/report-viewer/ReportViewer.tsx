...
export function ReportViewer({
  ...
  loading = false,
  error = null,
  emptyMessage = 'No report is available.',
  ...
  React.useEffect(() => {
    const documentElement =
      reportRef.current?.querySelector<HTMLElement>('.printout-document') ??
      (reportRef.current?.firstElementChild as HTMLElement | null);
    if (!documentElement || typeof ResizeObserver === 'undefined') return;
    const update = () =>
      setTotalPages(Math.max(1, Math.ceil(documentElement.offsetHeight / pageHeight)));
    update();
  ...
  const goToPage = (page: number) => {
    ...
    scrollRef.current?.scrollTo({
      top: ((next - 1) * pageHeight * zoom) / 100,
      behavior: 'smooth',
    });
  };
  ...
  const findInReport = () => {
    ...
    if (!query) return;
    [...(reportRef.current?.querySelectorAll<HTMLElement>('p, td, [data-report-search]') ?? [])]
  ...
  const secondary = (
    <>
      <Tooltip title="Presentation mode">
        <IconButton size="small" onClick={fullscreen}>
      ...
      <Tooltip title="Additional actions">
        <IconButton size="small" onClick={(event) => setOverflowAnchor(event.currentTarget)}>
  ...
  const viewer = (
    <Box
      ...
      aria-label={title}
      sx={{
        height: variant === 'dialog' ? '100dvh' : '100%',
        minHeight: 0,
        display: 'grid',
    ...
    >
      ...
      >
        ...
        {!compact ? (
          <Button sx={toolbarButtonSx} onClick={(event) => setZoomAnchor(event.currentTarget)}>
            Options
          </Button>
        ) : null}
        <Box sx={{ flex: 1 }} />
          ...
          <Stack direction="row" spacing={0.15}>
            <Tooltip title="Reading mode">
              <IconButton size="small" color="primary">
                <AutoStoriesOutlined sx={{ fontSize: 18 }} />
              </IconButton>
            </Tooltip>
            <Tooltip title="Reload">
              <IconButton size="small" color="primary" onClick={() => void onReload?.()}>
                <RefreshOutlined sx={{ fontSize: 18 }} />
              </IconButton>
            </Tooltip>
            <Tooltip title="Open presentation">
              <IconButton size="small" color="primary" onClick={fullscreen}>
      ...
      >
        <Stack direction="row" spacing={0.25} sx={{ alignItems: 'center', minWidth: 0 }}>
          <Tooltip title="Toggle thumbnails">
            <IconButton
              size="small"
              color={thumbnailsOpen ? 'primary' : 'default'}
              onClick={() => setThumbnailsOpen((value) => !value)}
            >
              <ViewSidebarOutlined sx={{ fontSize: 19 }} />
            </IconButton>
          </Tooltip>
          ...
          {searchOpen ? (
            <TextField
              ...
              onKeyDown={(event) => {
                if (event.key === 'Enter') findInReport();
              }}
              sx={{
                width: { xs: 78, sm: 140 },
                '& .MuiInputBase-root': { height: 27, fontSize: 11.5 },
              }}
            />
          ) : null}
          <IconButton
          ...
          <TextField
            ...
            onChange={(event) => goToPage(Number(event.target.value) || 1)}
              slotProps={{
                htmlInput: { 'aria-label': 'Current page', min: 1, max: totalPages },
              }}
            sx={{
              width: 48,
              '& .MuiInputBase-root': { height: 27, fontSize: 12 },
        ...
        <Stack direction="row" spacing={0.25} sx={{ alignItems: 'center' }}>
          ...
          <Button
            sx={{ ...toolbarButtonSx, minWidth: { xs: 112, sm: 145 }, bgcolor: '#e1e3e6' }}
            endIcon={<ArrowDropDown />}
            onClick={(event) => setZoomAnchor(event.currentTarget)}
          >
            {zoomMode}
          </Button>
        </Stack>
        <Stack direction="row" spacing={0.1} sx={{ justifySelf: 'end' }}>
          {!compact ? secondary : null}
        </Stack>
      </Paper>
      <Box
        sx={{
          minHeight: 0,
          display: 'grid',
          gridTemplateColumns: thumbnailsOpen
            ? { xs: '112px minmax(0, 1fr)', md: '188px minmax(0, 1fr)' }
            : '0 minmax(0, 1fr)',
          overflow: 'hidden',
        }}
      >
            ...
            <>
              <Stack
                ...
                sx={{
                  ...
                  px: 0.5,
                  bgcolor: '#eeeeef',
                  borderBottom: '1px solid #b8bdc4',
                }}
              >
                <IconButton size="small" color="primary">
                  <GridViewOutlined sx={{ fontSize: 17 }} />
                </IconButton>
                <IconButton size="small">
                  <ListOutlined sx={{ fontSize: 17 }} />
                </IconButton>
                <IconButton size="small">
              ...
              <Stack spacing={1.5} sx={{ p: 1.5, overflowY: 'auto' }}>
                {Array.from({ length: totalPages }, (_, index) => {
                  const page = index + 1;
                  return (
                    <Box key={page} onClick={() => goToPage(page)} sx={{ cursor: 'pointer' }}>
                      <Box
                        sx={{
                          width: { xs: 72, md: 108 },
                          aspectRatio: '210 / 297',
                          ...
                          bgcolor: '#fff',
                          border: '2px solid',
                          borderColor: currentPage === page ? '#315fa8' : '#9ba2ab',
                          boxShadow:
                            currentPage === page
                              ? '0 0 0 2px rgba(49,95,168,.2)'
                      ...
                      >
                        ...
                        <Stack spacing={0.5} sx={{ mt: 0.75 }}>
                          {[70, 92, 82, 95, 76, 88].map((width, line) => (
                            <Box
                              key={line}
                              sx={{ width: `${width}%`, height: 2, bgcolor: '#c2c7ce' }}
                            />
                          ))}
                        </Stack>
                      </Box>
        ...
        <Box
          ref={scrollRef}
          sx={{ minWidth: 0, overflow: 'auto', bgcolor: '#e7e8eb', p: { xs: 1, md: 2 } }}
        >
          {loading ? (
            ...
            <Alert severity="error" sx={{ m: 'auto', maxWidth: 520 }}>
              {error}
            </Alert>
          ) : children ? (
            <Box
              ref={reportRef}
              className="printout-preview-scale"
              sx={{
                width: 'max-content',
                mx: 'auto',
            ...
            <Alert severity="info" sx={{ m: 'auto', maxWidth: 520 }}>
              {emptyMessage}
            </Alert>
          )}
        </Box>
      ...
      <Menu anchorEl={zoomAnchor} open={Boolean(zoomAnchor)} onClose={() => setZoomAnchor(null)}>
        {(['Automatic Zoom', 'Page Width', 'Whole Page', '100%'] as ZoomMode[]).map((mode) => (
          <MenuItem
            key={mode}
  ...
  if (variant === 'dialog')
    return (
      <Dialog open={open} onClose={onClose} fullScreen disableRestoreFocus>
        {viewer}
      </Dialog>
    );
  return open ? viewer : <></>;
