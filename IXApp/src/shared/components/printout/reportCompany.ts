...
import type { PrintoutCompany } from './PrintoutDocument';
...
export const toPrintoutCompany = (entity: ReportCompanyInfo | undefined, companyCode: string, reportLogoSource?: string | null): PrintoutCompany => {
...
export async function fetchPrintoutCompany(companyCode: string, signal?: AbortSignal): Promise<PrintoutCompany> {
  ...
  if (!entity) return toPrintoutCompany(undefined, companyCode);
  ...
  try {
    ...
    if (!attachment) return toPrintoutCompany(entity, companyCode);
    const logo = await documentApi.previewBlob(attachment.id, signal);
    return toPrintoutCompany(entity, companyCode, await blobAsDataUrl(logo, signal));
  } catch (error) {
    if (signal?.aborted) throw error;
    return toPrintoutCompany(entity, companyCode);
  }
}
