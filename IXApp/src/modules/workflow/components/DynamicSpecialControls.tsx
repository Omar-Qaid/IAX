...
export function SignatureControl({ control, value, onChange, error, helperText, preview }: { control: RenderableControl; value: string; onChange: (value: string) => void; error?: boolean; helperText?: string; preview?: boolean }) {
  ...
  const savedImage = /^data:image\/(?:png|jpe?g|webp|gif|svg\+xml);base64,/i.test(value) ? value : null;
  ...
  return <FormControl error={error} required={control.required} fullWidth sx={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: 0.45 }}>
    {!control.hideLabel && <Typography component="label" sx={{ fontSize: 12.5, lineHeight: 1.2, fontWeight: 700 }}>{control.label}{control.required ? ' *' : ''}</Typography>}
    {!open && <Paper aria-label={`${control.label} signature preview`} title={canEdit ? saved.length ? 'Click to edit signature' : 'Click to add signature' : undefined} onClick={canEdit ? openDialog : undefined} variant="outlined" sx={{ height: 30, minHeight: 30, maxHeight: 30, flex: '0 0 30px', position: 'relative', boxSizing: 'border-box', display: 'grid', placeItems: 'center', px: 1, overflow: 'hidden', bgcolor: hasSavedSignature ? '#fff' : '#fafbfc', borderStyle: hasSavedSignature ? 'solid' : 'dashed', cursor: canEdit ? 'pointer' : 'default', userSelect: canEdit ? 'none' : 'auto', transition: 'border-color 140ms ease, box-shadow 140ms ease', '&:hover': canEdit ? { borderColor: 'primary.main', boxShadow: '0 2px 8px rgba(15,23,42,.09)' } : undefined, '&:hover .saved-signature-actions, &:focus-within .saved-signature-actions': { opacity: 1, pointerEvents: 'auto' } }}>
      {savedImage ? <Box component="img" src={savedImage} alt={`${control.label} signature`} sx={{ width: '100%', height: 26, objectFit: 'contain' }} /> : saved.length > 0 ? <Box component="svg" viewBox="0 0 600 220" sx={{ width: '100%', height: 26 }} aria-label="Saved signature">
