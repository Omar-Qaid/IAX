...
export interface FastTabField {
  name: string;
  label: string;
  type?: 'text' | 'select' | 'multiline' | 'date';
  required?: boolean;
  disabled?: boolean;
  options?: FastTabOption[];
  optionsGetter?: (values: Record<string, FastTabValue>) => FastTabOption[];
  valueGetter?: (values: Record<string, FastTabValue>) => FastTabValue;
  width?: number | string;
  rows?: number;
}
...
export interface FastTabsDialogProps<TValues extends Record<string, FastTabValue>> {
  ...
  canSave?: boolean;
  onFieldChange?: (name: string, value: FastTabValue, values: TValues) => Partial<TValues> | void;
  onCancel: () => void;
}

export function FastTabsDialog<TValues extends Record<string, FastTabValue>>({ open, title, sections, initialValues, resetKey, validate, onSubmit, saveLabel, saveAndOpenLabel, cancelLabel, closeLabel = 'Close', helpLabel = 'Help', placement = 'center', embedded = false, loading = false, loadError, canSave = true, onFieldChange, onCancel }: FastTabsDialogProps<TValues>): React.ReactElement {
  ...
  const content = <>
    ...
    <DialogContent sx={{ px: { xs: 1.5, sm: 3 }, py: 0, overflowY: 'auto', overflowX: 'hidden', '&::-webkit-scrollbar': { width: 8 }, '&::-webkit-scrollbar-thumb': { bgcolor: '#999', borderRadius: 4 } }}>
      {loading && <Box sx={{ py: 1.5, display: 'grid', placeItems: 'center' }}><CircularProgress size={22} /></Box>}
