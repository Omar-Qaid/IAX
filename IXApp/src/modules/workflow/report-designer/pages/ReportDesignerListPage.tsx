...
const statusKey = (status: ReportDesignerSummary['status']): string => {
...
export function WorkflowReportDesignerListPage(): React.ReactElement {
  ...
  const openTemplate = async (templateId: number) => {
    try {
      const template = await reportDesignerApi.get(templateId);
  ...
  const runConfirmedAction = async () => {
    ...
    setSaving(true);
    try {
      if (confirmAction === 'publish') await reportDesignerApi.publish(selected.templateId);
      if (confirmAction === 'archive') await reportDesignerApi.archive(selected.templateId);
      if (confirmAction === 'delete') await reportDesignerApi.deleteDraft(selected.templateId);
