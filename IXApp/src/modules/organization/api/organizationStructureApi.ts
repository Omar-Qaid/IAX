...
export interface WorkerOrganizationAssignment {
  assignmentId: number;
  workerId: number;
...
export const organizationStructureApi = {
  ...
  async reportingHierarchies(signal?: AbortSignal) {
    return (await apiClient.get<ReportingHierarchy[]>(`${base}/reporting-hierarchies`, { signal }))
      .data;
  },
  async createReportingHierarchy(payload: Omit<ReportingHierarchy, 'id'>) {
    return (await apiClient.post<number>(`${base}/reporting-hierarchies`, payload)).data;
  },
  async updateReportingHierarchy(id: number, payload: Omit<ReportingHierarchy, 'id'>) {
    return (await apiClient.put<number>(`${base}/reporting-hierarchies/${id}`, payload)).data;
  },
  async reportingLines(hierarchyId: number, asOf: string, signal?: AbortSignal) {
    return (
      await apiClient.get<PositionReportingLine[]>(
        `${base}/reporting-hierarchies/${hierarchyId}/lines`,
        { params: { asOf }, signal }
      )
    ).data;
  },
  async createReportingLine(payload: Omit<PositionReportingLine, 'id'>) {
    return (await apiClient.post<number>(`${base}/reporting-lines`, payload)).data;
  },
  async updateReportingLine(
    id: number,
    payload: Omit<PositionReportingLine, 'id' | 'reportingHierarchyId'>
  ) {
    return (await apiClient.put<number>(`${base}/reporting-lines/${id}`, payload)).data;
  },
  async closeReportingLine(id: number, validTo: string) {
    return (await apiClient.put<number>(`${base}/reporting-lines/${id}/close`, { validTo })).data;
  },
  async createRole(role: Omit<OrganizationRole, 'id'>) {
    return (await apiClient.post<number>(`${base}/roles`, role)).data;
  },
  async updateRole(id: number, role: Omit<OrganizationRole, 'id'>) {
    return (await apiClient.put<number>(`${base}/roles/${id}`, role)).data;
  },
  async deactivateRole(id: number) {
    return (await apiClient.delete<number>(`${base}/roles/${id}`)).data;
  },
  async workerAssignments(workerId: number, asOf: string, signal?: AbortSignal) {
    return (
      await apiClient.get<WorkerOrganizationAssignment[]>(
        `${base}/workers/${workerId}/assignments`,
        { params: { asOf }, signal }
      )
    ).data;
  },
  async assignWorker(payload: {
    workerId: number;
    positionId: number;
    validFrom: string;
    validTo: string | null;
    ...
    return (await apiClient.post<number>(`${base}/assignments`, payload)).data;
  },
  async updateWorkerAssignment(
    id: number,
    payload: { positionId: number; validFrom: string; validTo: string | null; isPrimary: boolean }
  ) {
    return (await apiClient.put<number>(`${base}/assignments/${id}`, payload)).data;
  },
  async closeAssignment(id: number, validTo: string) {
    return (await apiClient.put<number>(`${base}/assignments/${id}/close`, { validTo })).data;
  },
  ...
  async nodes(hierarchyId: number, asOf: string, signal?: AbortSignal) {
    return (
      await apiClient.get<OrganizationHierarchyNode[]>(`${base}/hierarchies/${hierarchyId}/nodes`, {
        params: { asOf },
        signal,
      })
    ).data;
  ...
  async createHierarchyWithRootNode(
    hierarchy: Omit<OrganizationHierarchy, 'id'> & {
      organizationUnitId: number;
      validFrom: string;
      validTo: string | null;
    }
