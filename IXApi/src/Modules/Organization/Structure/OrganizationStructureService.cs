...
using static IAX.IXApi.Modules.Organization.Structure.OrganizationPeriod;

namespace IAX.IXApi.Modules.Organization.Structure;

public sealed class OrganizationStructureService(IOrganizationDataContext db, ICompanyExecutionContext company) : IOrganizationDirectory
{
    ...
    private async Task<long> AssignAsync(AssignWorker request, long? excludedAssignmentId, CancellationToken ct)
    {
        ...
        var assignment = new HcmWorkerOrganizationAssignment { DataAreaId = Company, HcmWorkerId = request.WorkerId,
            PositionId = position.RecId, OrganizationUnitId = position.OrganizationUnitId, ValidFrom = request.ValidFrom, ValidTo = request.ValidTo,
