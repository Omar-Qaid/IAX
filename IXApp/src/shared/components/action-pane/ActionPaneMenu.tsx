...
export function ActionPaneMenu({ label, icon, actions, disabled }: ActionPaneMenuProps) {
  ...
  return <>
    <Button size="small" variant="text" startIcon={icon} endIcon={<ExpandMoreIcon />} onClick={openMenu} disabled={disabled || visible.length === 0} aria-haspopup="menu" aria-expanded={Boolean(anchorEl)} sx={actionPaneControlSx}>{label}</Button>
