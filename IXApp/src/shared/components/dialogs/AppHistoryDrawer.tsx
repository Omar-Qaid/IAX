        ...
        <TimelineItem>
            ...
            <TimelineContent sx={{ py: '12px', px: 2 }}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.8rem', mb: 1 }}>
                    {t('common.by', 'By')} <Box component="span" sx={{ color: 'text.primary', fontWeight: 500 }}>{user}</Box>
                </Typography>
...
export const AppHistoryDrawer: React.FC<AppHistoryDrawerProps> = ({
    open, onClose, history, loading = false, page = 1, observerTarget, title, onRefresh
}) => {
    const { t } = useTranslation();
            ...
            <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                ...
                <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 1 }}>
                    {loading && page === 1 ? (
                        ...
                        <>
                            ...
                            {observerTarget && <div ref={observerTarget} style={{ height: 20, marginBottom: 20 }} />}
                            {loading && page > 1 && (
                ...
                <Box sx={{ py: 0.75, px: 2, borderTop: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
                    <Typography variant="caption" color="text.secondary">
                        {t('common.total_activities', { count: history.length, defaultValue: `Total activities: ${history.length}` })}
                    </Typography>
                </Box>
            </Box>
        </Drawer>
