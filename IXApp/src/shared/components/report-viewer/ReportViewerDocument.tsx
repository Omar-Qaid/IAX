...
import { useReportViewerPagination } from './ReportViewerPaginationContext';
...
export interface ReportViewerDocumentProps {
...
export function PrintoutHeader({ company, title, reference, reportDate, status, generatedBy, criteria, config }: { company: reportCompany; title: string; reference?: string | null; reportDate?: string | Date | null; status?: string | null; generatedBy?: string | null; criteria?: React.ReactNode; config?: PrintoutHeaderConfig }): React.ReactElement {
  const { t, currentLanguage, isRtl } = useAppTranslation();
  const pagination = useReportViewerPagination();
...
export function PrintoutFooter({ company, reference, generatedBy, generatedAt, config, showPageNumber = true }: { company: reportCompany; reference?: string | null; generatedBy?: string | null; generatedAt?: string | Date | null; config?: PrintoutFooterConfig; showPageNumber?: boolean }): React.ReactElement {
...
export function ReportViewerDocument({ company, title, reference, reportDate, status, generatedBy, generatedAt, criteria, headerConfig, footerConfig, pageSettings, header, children, footer, paperSize = 'A4', orientation = 'portrait', margin = 'normal', showHeader = true, showBody = true, showFooter = true, showPageNumber = true }: ReportViewerDocumentProps): React.ReactElement {
