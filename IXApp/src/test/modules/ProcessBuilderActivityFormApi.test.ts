...
const mocks = vi.hoisted(() => ({
  ...
  requestMappingList: vi.fn(),
  requestMappingCreate: vi.fn(),
  requestMappingUpdate: vi.fn(),
  requestMappingDelete: vi.fn(),
  activityMappingList: vi.fn(),
  activityMappingCreate: vi.fn(),
  activityMappingUpdate: vi.fn(),
  activityMappingDelete: vi.fn(),
...
vi.mock('@modules/workflow/api/wfVariableMappingApis', () => ({
  wfRequestMappingVariableApi: {
    list: mocks.requestMappingList,
    create: mocks.requestMappingCreate,
    update: mocks.requestMappingUpdate,
    delete: mocks.requestMappingDelete,
  },
  wfActivityMappingVariableApi: {
    list: mocks.activityMappingList,
    create: mocks.activityMappingCreate,
    update: mocks.activityMappingUpdate,
    delete: mocks.activityMappingDelete,
  },
}));
...
beforeEach(() => {
  ...
  mocks.controlTypeList.mockResolvedValue([
    { recId: 2, code: 'DROPDOWN', name: 'Dropdown', controlType: 'dropdown-manual' },
  ]);
  ...
  mocks.requestMappingList.mockResolvedValue([]);
  mocks.activityMappingList.mockResolvedValue([]);
  mocks.transitionList.mockResolvedValue([]);
  mocks.apiGet.mockResolvedValue({
    data: {
      success: true,
      data: { mode: 'manual', manual: true, available: true, previewCode: null, message: null },
    },
  });
...
describe('Process Builder Activity Form backend integration', () => {
  ...
  it.each(['full', 'activities'] as const)(
    ...
    async (mode) => {
      ...
      await save(document);
      expect(mocks.activityUpdate).toHaveBeenLastCalledWith(
        expect.objectContaining({ sysNotificationTemplateId: 23 })
      );
      configured.sysNotificationTemplateId = null;
      await save(document);
      expect(mocks.activityUpdate).toHaveBeenLastCalledWith(
        expect.objectContaining({ sysNotificationTemplateId: null })
      );
    }
  );
  it.each(['request', 'activity'] as const)(
    'loads and saves relative date bounds for %s controls',
    async (kind) => {
      ...
      const control =
        kind === 'request'
          ? document.requestControls[0]
          : document.steps[0].activities[0].controls[0];
      ...
      expect(
        kind === 'request' ? mocks.requestValidationUpdate : mocks.activityValidationUpdate
  ...
  it.each(['full', 'transitions'] as const)(
    'blocks %s saves before writing unsupported compound conditions',
    async (mode) => {
      const document = await loadProcessBuilder(1);
      ...
      await expect(
        mode === 'full' ? saveProcessBuilder(document) : saveProcessTransitions(document)
      ).rejects.toThrow('AND/OR rule groups are draft only');
  ...
  it.each(['request', 'activity'] as const)(
    'preserves inactive options hidden from the %s editor',
    async (kind) => {
      const inactive = {
        ...option,
        recId: 51,
      ...
      if (kind === 'request') {
        mocks.requestControlList.mockResolvedValue([{ ...activityControl, processId: 1 }]);
        mocks.requestOptionList.mockResolvedValue(
          [option, inactive].map((item) => ({ ...item, requestControlId: 30 }))
        );
      } else {
      ...
      expect(
        kind === 'request' ? mocks.requestOptionDelete : mocks.activityOptionDelete
    ...
    mocks.requestOptionList.mockResolvedValue([
      {
        ...option,
        requestControlId: 30,
        extendedProperties: JSON.stringify({ allowFileUpload: true, sendAlert: true }),
      },
    ]);
    const document = await loadProcessBuilder(1);
  ...
  it('remaps generated graph identities into both full-save transition trigger kinds', async () => {
    ...
    document.requestControls = [
      { ...sourceControl, id: 'new-control', options: [], optionIds: [], validations: [] },
    ];
    document.transitions = (['activity', 'requestControl'] as const).map(
      (triggerSource, index) => ({
        id: `new-transition-${index}`,
        name: 'Next',
        sourceStepId: 'new-step',
        targetStepId: 'new-step',
        ...
        active: true,
        triggerSource,
        triggerId: triggerSource === 'activity' ? 'new-activity' : 'new-control',
      })
    );
    ...
    expect(mocks.transitionCreate).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        processId: 1,
        variableId: 101,
  ...
  it('rejects a foreign step during full-save preflight', async () => {
    ...
    await expect(saveProcessBuilder(document)).rejects.toThrow(
      'stale or belongs to another parent'
    );
    expect(mocks.processUpdate).not.toHaveBeenCalled();
  ...
  it('keeps activity aliases and features aligned when blank options are omitted', async () => {
    ...
    expect(mocks.activityOptionUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        recId: 50,
        value: 'APP',
        nameAlias: 'Approval alias',
      })
    );
    ...
    document.requestControls[0].optionFeatureConfigurations = [
      {
        ...
        visibleControlIds: ['missing-target'],
      },
    ];
  ...
  it('remaps new option visibility targets to generated IDs', async () => {
    ...
    source.optionFeatureConfigurations = [
      {
        ...
        performerIds: [],
        showOtherControls: true,
        visibleControlIds: ['new-target'],
      },
    ];
    document.requestControls.push({
      ...source,
      id: 'new-target',
      code: 'TARGET',
      label: 'Target',
    ...
    await saveProcessRequestControls(document);
    expect(
      JSON.parse(mocks.requestOptionUpdate.mock.calls.at(-1)![0].extendedProperties)
        .visibleControlIds
    ).toEqual(['99']);
  });
  it.each(['999', 'deleted-draft-control'])(
    'rejects missing visibility source %s before request form writes',
    async (sourceId) => {
      mocks.requestControlList.mockResolvedValue([{ ...activityControl, processId: 1 }]);
      const document = await loadProcessBuilder(1);
      document.requestControls[0].visibilityCondition = {
        variableId: sourceId,
        operator: '=',
        value: 'Yes',
      };
  ...
  it('remaps a valid new visibility source to its generated control ID', async () => {
    ...
    await saveProcessRequestControls(document);
    const targetWrites = mocks.requestControlUpdate.mock.calls
      .map(([record]) => record)
  ...
  it.each(['step', 'requestControl', 'activity'] as const)(
    'rejects an unavailable transition %s before transition mutations',
    async (missing) => {
      mocks.variableList.mockResolvedValue([
        { recId: 60, processId: 1, dataTypeId: 2, name: 'Choice', sortOrder: 10, isActive: true },
      ]);
      ...
      document.transitions = [
        {
          ...
          name: '',
          sourceStepId: '',
          targetStepId: missing === 'step' ? '999' : '10',
          variableId: '60',
          operator: '=',
          operatorId: '3',
          value: 'A',
          sortOrder: 10,
          active: true,
          triggerSource: missing === 'step' ? 'none' : missing,
          triggerId: '999',
        },
      ];
      mocks.transitionList.mockResolvedValue([{ recId: 70, processId: 1 }]);
      await expect(saveProcessTransitions(document)).rejects.toThrow(
        'outside the current process document'
      );
      expect(mocks.transitionDelete).not.toHaveBeenCalled();
      expect(mocks.transitionCreate).not.toHaveBeenCalled();
      expect(mocks.transitionUpdate).not.toHaveBeenCalled();
  ...
  it.each(['request', 'activity'] as const)(
    'rejects malformed stored %s metadata before control writes',
    async (kind) => {
      ...
      const document = await loadProcessBuilder(1);
      await expect(
        kind === 'request' ? saveProcessRequestControls(document) : saveProcessActivities(document)
      ).rejects.toThrow('ExtendedProperties');
      expect(
        kind === 'request' ? mocks.requestControlUpdate : mocks.activityControlUpdate
      ).not.toHaveBeenCalled();
      expect(
        kind === 'request' ? mocks.requestControlDelete : mocks.activityControlDelete
      ).not.toHaveBeenCalled();
    }
  );

  it.each(['request', 'activity'] as const)(
    'rejects stale %s option IDs before control mutations',
    async (kind) => {
      if (kind === 'request') {
        mocks.requestControlList.mockResolvedValue([{ ...activityControl, processId: 1 }]);
        mocks.requestOptionList.mockResolvedValue([{ ...option, requestControlId: 30 }]);
      ...
      const mutations =
        kind === 'request'
          ? [
              mocks.requestControlDelete,
              mocks.requestControlUpdate,
              mocks.requestControlCreate,
              mocks.requestValidationDelete,
              mocks.requestValidationUpdate,
              mocks.requestOptionDelete,
              mocks.requestOptionUpdate,
            ]
          : [
              mocks.activityControlDelete,
              mocks.activityControlUpdate,
              mocks.activityControlCreate,
              mocks.activityValidationDelete,
              mocks.activityValidationUpdate,
              mocks.activityOptionDelete,
              mocks.activityOptionUpdate,
            ];
  ...
  it.each(['request', 'activity'] as const)(
    'preserves %s option identities through draft recovery and replacement',
    async (kind) => {
      ...
      const control =
        kind === 'request' ? server.requestControls[0] : server.steps[0].activities[0].controls[0];
      ...
      try {
        ...
        expect(update).toHaveBeenCalledExactlyOnceWith(
          expect.objectContaining({ recId: 51, name: 'Decline', value: 'REJ' })
        );
        expect(create).toHaveBeenCalledExactlyOnceWith(
          expect.objectContaining({ recId: 0, name: 'Return', value: 'Return' })
        );
      } finally {
  ...
  it('restores activity option features after saving and reopening the process', async () => {
    const features = {
      requireFileUpload: true,
      sendAlertMessage: true,
      alertMessage: 'Evidence required',
      performerIds: ['2'],
      showOtherControls: true,
      visibleControlIds: ['30'],
    };
    ...
    expect(reopened.steps[0].activities[0].controls[0].optionFeatureConfigurations).toEqual([
      features,
    ]);
  });
  ...
  it('maps renamed workflow settings without sending removed fields', async () => {
    ...
    expect(saved).toMatchObject({
      mandatoryDocuments: true,
      isAutoPassEnabled: true,
      autoPassAfterHours: 48,
      isEmailNotificationEnabled: false,
      isWhatsAppNotificationEnabled: true,
      canViewPreviousSteps: true,
      canViewPreviousDocuments: true,
    });
  ...
  it('keeps server notification settings when restoring a draft made before these fields existed', async () => {
    mocks.activityList.mockResolvedValue([
      { ...activity, isEmailNotificationEnabled: true, canViewPreviousDocuments: true },
    ]);
    const server = await loadProcessBuilder(1);
  ...
  it('loads and saves activity controls, options, and validations', async () => {
    ...
    expect(JSON.parse(activityWrite.extendedProperties)).toMatchObject({
      integration: { key: 'keep' },
    });
    expect(JSON.parse(activityWrite.validationRules)).toMatchObject({
      externalRule: { enabled: true },
    });

    expect(mocks.activityControlUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        recId: 30,
        activityId: 20,
        processId: 1,
    ...
    expect(mocks.activityValidationUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        recId: 40,
        activityControlId: 30,
        errorMessage: 'Decision is required',
  ...
  it.each(['EQ', 'OP5'])(
    'loads and saves request controls, options, validations, and transitions with %s',
    async (operatorCode) => {
      ...
      for (const [write] of mocks.requestControlUpdate.mock.calls) {
        expect(JSON.parse(write.extendedProperties)).toMatchObject({
          integration: { key: 'keep' },
        });
        expect(JSON.parse(write.validationRules)).toMatchObject({
          externalRule: { enabled: true },
        });
      }
      ...
      expect(mocks.requestControlUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          recId: 31,
          processId: 1,
          controlId: 4,
      ...
      expect(mocks.requestControlUpdate).toHaveBeenLastCalledWith(
        expect.objectContaining({
          extendedProperties: expect.not.stringContaining('optionFeatureConfigurations'),
        })
      );
      expect(mocks.requestValidationUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          recId: 41,
          requestControlId: 31,
          validationType: 'minSelected',
      ...
      expect(mocks.requestOptionUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          recId: 51,
          requestControlId: 31,
          name: 'One',
      ...
      expect(mocks.transitionUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          recId: 70,
          requestControlId: 31,
          activityId: null,
