...
const navigationMetadata: RouteMetadata[] = Object.values(MODULE_NAV_CONFIGS).flatMap(module =>
  module.sections.flatMap(section => section.links.flatMap(link => link.path ? [{
    path: link.path,
    breadcrumbs: [home, { labelKey: module.label, path: module.defaultPath }, { labelKey: link.label }],
  }] : [])),
);
