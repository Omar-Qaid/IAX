...
namespace IAX.IXApi.Modules.ERP.Foundation.LogisticsAddresses
{
    ...
    public class LogisticsPostalAddressController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetPartyAddresses(long partyId)
        {
            ...
            var postalOnly = partyLocations.Where(x => x.IsPostalAddress == IAX.IXApi.Modules.ERP.Common.NoYes.Yes).ToList();
            ...
            var dtos = postalAddresses.Select(p => {
                ...
                return new AddressInfoDto
                {
                    ...
                    Address = p.Address,
                    Primary = pLoc?.IsPrimary == IAX.IXApi.Modules.ERP.Common.NoYes.Yes,
                    Street = p.Street,
                    City = p.City,
                    State = p.State,
                    ZipCode = p.ZipCode,
