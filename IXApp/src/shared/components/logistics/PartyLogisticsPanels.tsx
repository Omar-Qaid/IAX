...
import { TabularDetailPanel } from '@shared/components/data-grid/TabularDetailPanel';
...
const requireData = <T,>(response: ApiResponse<T>): T => {
  if (!response.success || response.data == null)
    throw new ApiError(
      response.message || 'The party logistics response did not contain data.',
      500
    );
...
const partyLogisticsApi = {
  async addresses(partyId: number, signal?: AbortSignal): Promise<StoredPostalAddress[]> {
    const response = await apiClient.get<ApiResponse<StoredPostalAddress[]>>(
  ...
  async contacts(partyId: number, signal?: AbortSignal): Promise<StoredElectronicAddress[]> {
    const response = await apiClient.get<ApiResponse<StoredElectronicAddress[]>>(
...
export function PartyPostalAddressPanel({
  partyId,
  editing,
  storageKey,
  ...
  const query = useQuery({
    queryKey: key,
    queryFn: ({ signal }) => partyLogisticsApi.addresses(validPartyId!, signal),
    enabled: validPartyId !== null,
  });
  ...
  const columns = useMemo<ColumnDef<StoredPostalAddress>[]>(
    () => [
      ...
      {
        field: 'address',
        headerName: t('legalEntities.addresses.address'),
        minWidth: 260,
        flex: 1,
      },
      {
  ...
  return (
    <>
      ...
      <TabularDetailPanel
        ...
        disabled={!editing || validPartyId === null || query.isLoading}
        onAdd={() => {
          if (validPartyId === null) return;
          setSelectedIds([]);
          setDrawerOpen(true);
        }}
        onRemove={() =>
          selected &&
          validPartyId !== null &&
          void run(() => partyLogisticsApi.deleteAddress(validPartyId, selected.location))
        }
        actions={[
          {
            id: 'edit',
            ...
            disabled: !selected || validPartyId === null,
      ...
      <LogisticsPostalAddressDrawer
        open={drawerOpen && validPartyId !== null}
        onClose={() => setDrawerOpen(false)}
        onSave={(draft) =>
          validPartyId !== null &&
          void run(() =>
            partyLogisticsApi.saveAddress(
              validPartyId,
              selected ? { ...draft, id: selected.id, location: selected.location } : draft,
              Boolean(selected)
            )
          )
        }
...
export function PartyElectronicAddressPanel({
  partyId,
  editing,
  storageKey,
}: PartyPanelProps): React.ReactElement {
  ...
  const query = useQuery({
    queryKey: key,
    queryFn: ({ signal }) => partyLogisticsApi.contacts(validPartyId!, signal),
    enabled: validPartyId !== null,
  });
  const rows = query.data ?? [];
  const [selectedIds, setSelectedIds] = useState<(string | number)[]>([]);
    ...
    <>
      ...
      <TabularDetailPanel
        ...
        onRemove={() =>
          selected &&
          validPartyId !== null &&
          void run(() => partyLogisticsApi.deleteContact(validPartyId, selected.location))
        }
        actions={[
          {
            id: 'edit',
