...
export function ListDetailsLayout({ listPane, header, sections, values, editing, yesLabel, noLabel, onChange, listWidth = 264, listMinWidth = 176, listMaxWidth = 520, listResizable = true, listPaneVisible = true, onListPaneClose, listWidthStorageKey }: ListDetailsLayoutProps): React.ReactElement {
  ...
  return <Box sx={{ minHeight: 0, flex: 1, display: 'flex', gap: 0, overflow: 'hidden', fontFamily: d365.fontFamily, color: d365.text }}>
    ...
    {listPaneVisible && <Box sx={{ width: currentListWidth, flexShrink: 0, display: { xs: 'none', md: 'block' }, bgcolor: d365.surface, border: `1px solid ${d365.border}`, borderRadius: `${d365.sectionRadius}px`, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,.14)', willChange: resizing ? 'width' : 'auto', transition: resizing ? 'none' : 'width 160ms ease-out' }}>{listPane}</Box>}
    ...
    <Box sx={{ minWidth: 0, flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <Box sx={{ minHeight: 0, flex: 1, overflowY: 'auto', pr: '28px', scrollbarWidth: 'thin' }}>
        {sections.map((section) => <Accordion key={section.id} defaultExpanded={section.defaultExpanded ?? true} disableGutters elevation={0} sx={{ mb: `${d365.sectionGap}px !important`, border: `1px solid ${d365.border}`, borderRadius: `${d365.sectionRadius}px !important`, bgcolor: d365.surface, overflow: 'hidden', '&::before': { display: 'none' } }}>
          <AccordionSummary expandIcon={<ExpandMoreIcon sx={{ fontSize: 16 }} />} sx={{ px: 1, minHeight: `${d365.sectionHeaderHeight}px !important`, borderBottom: 1, borderColor: d365.border, '&.Mui-expanded': { minHeight: `${d365.sectionHeaderHeight}px !important` }, '& .MuiAccordionSummary-content, & .MuiAccordionSummary-content.Mui-expanded': { my: 0.5 }, '& .MuiAccordionSummary-expandIconWrapper': { border: `1px solid ${d365.border}`, borderRadius: d365.radius, p: '2px' } }}><Typography sx={{ fontSize: 14, fontWeight: 600 }}>{section.title}</Typography></AccordionSummary>
            ...
            {section.content ?? <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,1fr)', lg: section.gridTemplateColumns ?? `repeat(${section.columns ?? Math.min(5, section.groups?.length ?? 1)}, minmax(120px,1fr))` }, justifyContent: section.gridTemplateColumns ? 'start' : 'space-between', columnGap: section.columnGap ?? `${d365.fieldGap}px`, rowGap: 1 }}>
              {(section.groups ?? []).map((group) => <Box key={group.id} sx={{ minWidth: 0, width: group.width, gridColumn: group.column }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: group.columns ? `repeat(${group.columns}, minmax(0, 1fr))` : '1fr', columnGap: 2, rowGap: '12px' }}>{group.fields.map((field) => {
                  ...
                  return <Box key={field.name} sx={{ minWidth: 0, width: field.width ?? '100%', gridColumn: field.column, gridRow: field.row }}><Typography noWrap title={field.label} sx={{ mb: '6px', fontSize: d365.labelFontSize, lineHeight: 1.2, color: d365.text }}>{field.label}</Typography>
                    {field.type === 'boolean' ? <Box sx={{ display: 'flex', alignItems: 'center', height: d365.controlHeight }}><Switch size="small" checked={Boolean(value)} disabled={!editable} onChange={(_, checked) => onChange(field.name, checked)} sx={d365SwitchSx} /><Typography sx={{ fontSize: d365.fontSize }}>{value ? yesLabel : noLabel}</Typography></Box>
                    ...
                    : editable ? <TextField type={field.type === 'number' ? 'number' : 'text'} multiline={field.multiline} rows={field.multiline ? (field.rows ?? 4) : undefined} value={value ?? ''} onChange={(event) => onChange(field.name, field.type === 'number' ? Number(event.target.value) : event.target.value)} sx={field.multiline ? multilineFieldSx : editFieldSx} />
...
function ViewField({ value, numeric = false }: { value: DetailValue | undefined; numeric?: boolean }): React.ReactElement {
  return <Box sx={{ width: '100%', minHeight: d365.controlHeight, display: 'flex', alignItems: 'center', justifyContent: numeric ? 'flex-end' : 'flex-start', borderBottom: `1px solid ${d365.darkBorder}`, px: 0.5, overflow: 'hidden' }}><Typography noWrap sx={{ fontSize: d365.fontSize }}>{String(value ?? '')}</Typography></Box>;
}
