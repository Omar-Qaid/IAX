            ...
            {section.content ?? <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,1fr)', lg: section.gridTemplateColumns ?? `repeat(${section.columns ?? Math.min(5, section.groups?.length ?? 1)}, minmax(120px,1fr))` }, justifyContent: section.gridTemplateColumns ? 'start' : 'space-between', columnGap: section.columnGap ?? `${d365.fieldGap}px`, rowGap: 1 }}>
              {(section.groups ?? []).map((group) => <Box key={group.id} sx={{ minWidth: 0, width: group.width, gridColumn: group.column }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: group.columns ? `repeat(${group.columns}, minmax(0, 1fr))` : '1fr', columnGap: 2, rowGap: '12px' }}>{group.fields.map((field) => {
                  ...
                  return <Box key={field.name} sx={{ minWidth: 0, width: field.width ?? '100%', gridColumn: field.column, gridRow: field.row }}>{sectionTitle && <Typography sx={{ mt: '10px', mb: '8px', fontFamily: d365.fontFamily, fontSize: 12, lineHeight: 1.2, fontWeight: 700, textTransform: 'uppercase', color: '#111' }}>{sectionTitle}</Typography>}<Typography noWrap title={fieldLabel} sx={{ mb: '6px', fontFamily: d365.fontFamily, fontSize: legalEntity ? 12 : d365.labelFontSize, lineHeight: 1.2, color: d365.text }}>{fieldLabel}</Typography>
                    ...
                    : <ViewField value={field.type === 'select' ? (field.options?.find((option) => option.value === String(value))?.label ?? value) : value} numeric={field.type === 'number'} legalEntity={legalEntity} disabled={field.disabled} linkStyle={field.linkStyle || (legalEntity && field.name === 'languageId')} />}
                  </Box>;
                })}</Box>
              </Box>)}
            </Box>}
...
function ViewField({ value, numeric = false, legalEntity = false, disabled = false, linkStyle = false }: { value: DetailValue | undefined; numeric?: boolean; legalEntity?: boolean; disabled?: boolean; linkStyle?: boolean }): React.ReactElement {
