...
import { hcmDepartmentApi, type HcmReferenceRecord } from '../api/hcmReferenceApi';
import { useNavigate } from 'react-router-dom';

const queryKey = ['simple-list', 'hcm-departments'] as const;

export function HcmDepartmentPage(): React.ReactElement {
  const { t, currentLanguage } = useAppTranslation();
  const { notifyError, notifySuccess } = useNotifications();
  const navigate = useNavigate();

  const columns = useMemo<ColumnDef<HcmReferenceRecord>[]>(() => [
    { field: 'code', headerName: 'hcmDepartments.fields.code', width: 140, pinned: 'left', editable: true },
  ...
  const config: EnterpriseListConfig<HcmReferenceRecord> = {
    ...
    crud: {
      ...
      onDelete: async (rows) => {
        try {
          await Promise.all(rows.map((row) => hcmDepartmentApi.delete(row)));
          await refresh();
    ...
    advancedFilter: {
      ...
      fieldLabel: t('hcmDepartments.fields.code'),
  ...
  return (
    <SimpleListPage
      title={t('hcmDepartments.title')}
      enterpriseConfig={config}
      dataSource={{
        type: 'remote',
        key: 'hcm-departments',
        load: (signal) => hcmDepartmentApi.list(signal),
      }}
      columns={columns}
      dataGridProps={{
        storageKey: 'organization.hcm-departments.reference-view',
        masterForm: true,
        hideSidebar: false,
        ...
        onRowSave: async (values, isNew) => {
          ...
          if (!record.code.trim()) throw new Error(t('validation.required', { field: t('hcmDepartments.fields.code') }));
          if (!record.name.trim()) throw new Error(t('validation.required', { field: t('fields.name') }));
          if (isNew || record.recId === 0) {
            await hcmDepartmentApi.create(record);
          } else {
            await hcmDepartmentApi.update(record);
          }
          await refresh();
