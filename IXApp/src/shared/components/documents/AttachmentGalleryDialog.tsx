...
const fileIcon = (item: DocumentDto) => {
  ...
  if (item.contentType === 'application/pdf' || extension === 'pdf')
    return <PictureAsPdfOutlined sx={{ color: '#c62828' }} />;
  if (extension === 'docx' || extension === 'doc')
    return <DescriptionOutlined sx={{ color: '#2563b9' }} />;
  if (['xlsx', 'xls', 'csv'].includes(extension))
    return <TableChartOutlined sx={{ color: '#16834a' }} />;
...
function SpreadsheetPreview({ sheets }: { sheets: Spreadsheet[] }): React.ReactElement {
  ...
  if (!sheet) return <Alert severity="info">{t('documents.previewMessages.emptyWorkbook')}</Alert>;
  return (
    <Stack sx={{ width: '100%', height: '100%', minHeight: 0 }}>
      {sheets.length > 1 && (
        <Tabs
          value={sheetIndex}
          onChange={(_, value: number) => setSheetIndex(value)}
          variant="scrollable"
          scrollButtons="auto"
          sx={{
            minHeight: 34,
            ...
            borderColor: 'divider',
            '& .MuiTab-root': { minHeight: 34, py: 0, fontSize: 11 },
          }}
        >
          {sheets.map((candidate) => (
            <Tab key={candidate.name} label={candidate.name} />
          ))}
        </Tabs>
      )}
      <Box sx={{ flex: 1, minHeight: 0, overflow: 'auto', bgcolor: '#fff' }}>
        ...
        {sheet.rows.length >= 500 && (
          <Alert severity="info">{t('documents.previewMessages.rowLimit')}</Alert>
        )}
      </Box>
    </Stack>
  );
...
function OfficePreview({ blob, item }: { blob: Blob; item: DocumentDto }): React.ReactElement {
    ...
    void (async () => {
      try {
        if (extension === 'docx') {
          ...
          const rows = (await blob.text())
            .split(/\r?\n/)
            .slice(0, 500)
            .filter((line) => line.length > 0)
            .map(parseCsvLine);
          if (active) setSheets([{ name: 'CSV', rows }]);
        } else throw new Error(t('documents.previewMessages.legacyOffice'));
      } catch (reason) {
        if (active)
          setError(
            reason instanceof Error ? reason.message : t('documents.previewMessages.renderFailed')
          );
      } finally {
  ...
  if (error)
    return (
      <Alert severity="info">
        {error} {t('documents.previewMessages.useOriginal')}
      </Alert>
    );
  if (wordHtml) {
...
function GalleryPreview({ item }: { item: DocumentDto }): React.ReactElement {
  ...
  if (item.kind === 'Note')
    return (
      <Typography sx={{ p: 3, whiteSpace: 'pre-wrap' }}>
        {item.notes || t('documents.previewMessages.noNote')}
      </Typography>
    );
  if (item.kind === 'URL')
    return item.url ? (
      <Button component="a" href={item.url} target="_blank" rel="noreferrer">
        {t('documents.openUrl')}
      </Button>
    ) : (
      <Alert severity="info">{t('documents.previewMessages.noUrl')}</Alert>
    );
  ...
  if (preview.isError || !preview.data || !url)
    return <Alert severity="error">{t('documents.previewMessages.loadFailed')}</Alert>;
  const extension = extensionOf(item);
  ...
  if (['docx', 'doc', 'xlsx', 'xls', 'csv'].includes(extension))
    return <OfficePreview blob={preview.data} item={item} />;
  return <Alert severity="info">{t('documents.previewMessages.unavailable')}</Alert>;
}
...
export function AttachmentGalleryDialog({
  ...
  refRecId,
  onClose,
}: AttachmentGalleryDialogProps): React.ReactElement {
  const { t, currentLanguage } = useAppTranslation();
  const rtl = currentLanguage.dir === 'rtl';
  ...
  const items = React.useMemo(
    () => documents.data?.items ?? (initialDocument ? [initialDocument] : []),
    [documents.data?.items, initialDocument]
  );
  const [selectedId, setSelectedId] = React.useState<number | null>(initialDocument?.id ?? null);
  React.useEffect(() => {
    if (open) setSelectedId(initialDocument?.id ?? null);
  }, [initialDocument?.id, open]);
  ...
  return (
    <Dialog
      ...
      fullWidth
      maxWidth="xl"
      aria-labelledby="attachment-gallery-title"
      slotProps={{ paper: { sx: { height: '88vh', maxHeight: 900 } } }}
    >
      <DialogTitle
        id="attachment-gallery-title"
        sx={{
          minHeight: 48,
          py: 0.75,
          px: 1.25,
      ...
      >
        ...
        >
          {current?.name || current?.fileName || t('documents.attachmentPreview')}
        </Typography>
        {current && (
        ...
        {current && (current.kind === 'File' || current.kind === 'Image') && (
          <>
            <Tooltip title={t('actions.download')}>
              <IconButton
                size="small"
                aria-label={t('documents.downloadNamed', { name: current.name })}
                onClick={() => void documentApi.download(current)}
              >
                <DownloadOutlined fontSize="small" />
              </IconButton>
            </Tooltip>
            <Tooltip title={t('documents.openNewTab')}>
              <IconButton
                size="small"
                aria-label={t('documents.openNamedNewTab', { name: current.name })}
                onClick={() => void documentApi.preview(current)}
              >
        ...
        <Tooltip title={t('actions.close')}>
          <IconButton size="small" aria-label={t('documents.closePreview')} onClick={onClose}>
      ...
      >
        ...
        >
          ...
          >
            {t('documents.attachmentsCount', { count: items.length })}
          </Typography>
          <List dense disablePadding>
            {items.map((item) => (
              <ListItemButton
                ...
                sx={{ mx: 0.5, mb: 0.25, px: 1, borderRadius: 0.75 }}
              >
                <Box sx={{ width: 32, display: 'grid', placeItems: 'center' }}>
                  {fileIcon(item)}
                </Box>
                <ListItemText
                  primary={item.name || item.fileName}
                  secondary={item.documentTypeName}
                  slotProps={{
                    primary: { noWrap: true, sx: { fontSize: 11.5, fontWeight: 600 } },
                    secondary: { noWrap: true, sx: { fontSize: 10 } },
                  }}
                />
              </ListItemButton>
            ))}
        ...
        >
          {documents.isLoading && !current ? (
            ...
            <Alert severity="info">{t('documents.noAttachments')}</Alert>
          )}
          {items.length > 1 && (
            <>
              <IconButton
                aria-label={t('documents.previousAttachment')}
                onClick={() => move(-1)}
                sx={{
                  position: 'absolute',
                  insetInlineStart: 18,
              ...
              <IconButton
                aria-label={t('documents.nextAttachment')}
                onClick={() => move(1)}
                sx={{
                  position: 'absolute',
                  insetInlineEnd: 18,
