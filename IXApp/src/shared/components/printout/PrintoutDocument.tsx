...
const formatReportDate = (value?: string | Date | null): string => {
  if (!value) return '';
...
function PrintoutPagePosition({ pageNumber, totalPages }: { pageNumber: number; totalPages: number }): React.ReactElement {
  return <>
    <Box component="span" className="printout-screen-page-position">Page {pageNumber} of {totalPages}</Box>
    <Box component="span" className="printout-print-page-position">Page <Box component="span" className="printout-page-number" /> of <Box component="span" className="printout-page-count" /></Box>
  </>;
}
    ...
    <Box className="printout-header">
      <Box sx={{ display: 'grid', gridTemplateColumns: '31mm minmax(0, 1fr) 58mm', alignItems: 'center', gap: 2, minHeight: '29mm', pb: 1.5, borderBottom: '2px solid #174f82' }}>
        ...
        <Box sx={{ minWidth: 0, textAlign: 'center' }}>
          ...
          {config?.period ? <Typography sx={{ mt: 0.25, fontSize: 9, color: '#486581' }}>Period: {config.period}</Typography> : null}
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'max-content minmax(0, 1fr)', columnGap: 0.75, rowGap: 0.4, textAlign: 'right', color: '#486581' }}>
          {company.companyCode ? <Typography sx={{ fontSize: 9.5, fontWeight: 700 }}>Company: {company.companyCode}</Typography> : null}
          <Box />
          {reference ? <><Typography sx={{ fontSize: 8.5, fontWeight: 700 }}>Report No:</Typography><Typography sx={{ fontSize: 8.5 }}>{reference}</Typography></> : null}
          {reportDate ? <><Typography sx={{ fontSize: 8.5, fontWeight: 700 }}>Date:</Typography><Typography sx={{ fontSize: 8.5 }}>{formatReportDate(reportDate)}</Typography></> : null}
          {status ? <><Typography sx={{ fontSize: 8.5, fontWeight: 700 }}>Status:</Typography><Typography sx={{ fontSize: 8.5 }}>{status}</Typography></> : null}
          {generatedBy ? <><Typography sx={{ fontSize: 8.5, fontWeight: 700 }}>Generated by:</Typography><Typography sx={{ fontSize: 8.5 }}>{generatedBy}</Typography></> : null}
      ...
      {criteria ? <Box sx={{ px: 1, py: 0.65, borderBottom: '1px solid #d9e2ec', color: '#486581', fontSize: 8.5 }}><Box component="span" sx={{ fontWeight: 700 }}>Criteria: </Box>{criteria}</Box> : null}
    </Box>
  );
}
    ...
    <Box className="printout-footer">
      <Box sx={{ pt: 1.25 }}>
        <Box sx={{ borderTop: '1px solid #9fb3c8', pt: 0.75, display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 1.5, alignItems: 'end', color: '#627d98' }}>
          <Box>
            <Typography sx={{ fontSize: 8.5, fontWeight: 700 }}>{config?.confidentialityText ?? 'Confidential / Internal Use Only'}</Typography>
          ...
          <Box sx={{ textAlign: 'right' }}>
            {config?.generatedAt ?? generatedAt ? <Typography sx={{ fontSize: 8 }}>Generated: {formatReportDate(config?.generatedAt ?? generatedAt)}</Typography> : null}
            {config?.generatedBy ?? generatedBy ? <Typography sx={{ fontSize: 8 }}>Printed by: {config?.generatedBy ?? generatedBy}</Typography> : null}
...
export function PrintoutDocument({ company, title, reference, reportDate, status, generatedBy, generatedAt, criteria, headerConfig, footerConfig, pageSettings, header, children, footer, paperSize = 'A4', orientation = 'portrait', margin = 'normal', showHeader = true, showBody = true, showFooter = true, showPageNumber = true }: PrintoutDocumentProps): React.ReactElement {
  ...
  return (
    <>
      ...
      <Box className="printout-document printout-color" dir={pageSettings?.direction ?? 'ltr'} sx={{ width: `${previewWidth}mm`, minHeight: `${previewHeight}mm`, mx: 'auto', p: `${padding}mm`, boxSizing: 'border-box', bgcolor: '#fff', color: '#102a43', fontFamily: 'Arial, "Segoe UI", sans-serif', boxShadow: '0 4px 24px rgba(15,23,42,.16)' }}>
